...
> docs/design/20260904-soak-72h-final-report.md
> 历史更新：2026-08-28 — 外部 L7 审计复审（§8）：四项整改核验通过，总评 6.5 → 7.5；剩余缺口见 §8.3
> 历史更新：2026-08-25 — 外部 L7 工程审计报告入库（见「外部 L7 工程审计」节）
...
## 8. 复审（2026-08-28）：整改核验 + 更新结论

> 触发：审计报告入库后（`9b93cc2`）仓库发生一轮系统性整改。本节逐条核验整改是否为"真修复"（非纸面），并更新评分与落地距离判断。所有验证均为命令级复现。

### 8.1 五项行动的整改状态

| # | 审计行动 | 状态 | 核验证据 |
|---|---|:---:|---|
| 1 | 私钥轮换 + 历史重写 + gitleaks | **✅ 收官** | 历史重写强推后 `git log --all --name-only --diff-filter=A \| grep 'key\.pem'` = **0**（原 11）；CA 整体烧毁轮换（`3192b1b`）；`.gitleaks.toml` 进 CI（`0c3f561`），**首轮全量扫描即抓出一个 live AI key**（泄露 #2，`689bbde`）——防线有效非摆设 |
| 2 | 覆盖率链修复 | **✅ 自洽可复算** | 徽章改 CI 实测 endpoint（`badge.json`，多次 `[skip ci]` 归档提交证明回流在跑）；明细与总量矛盾消除：astar 98.9% / KF 100% / motor 82.7%，总量 **89.3%**（原 84.5% 与全员 ≤61.9% 的明细数学矛盾）；`quality.sh` lcov 失败静默放行旁路已删除。修复过程挖出 3 个真 bug：`-fprofile-update=atomic` 治 gcda 多线程计数撕裂、CI 覆盖率目录解析不对齐（**CI 里覆盖率从未真正测过**）、`set -e` 误杀 |
| 3 | fail-fast + launch lint | **✅ 落地** | `sensor_factory.hpp` 静默 fallback 改为拒绝启动（带产线隐患说明注释，`80e426f`）；launch 语法检查进 CI |
| 4 | e2e 断言式测试 | **✅ 真断言** | `quality/src/test_e2e_behavior.cpp` 三场景：碰撞距离硬断言（`min_dist > 0.57m` 带失败诊断）、`stays_stopped(3s)`、断源 3s 降级守门（`4c3751b`），**挖修 4 个真 bug**——"车真的停了"从零到 CI 硬门禁 |
| 5 | 72h soak + systemd + 版本对齐 | ⏳ **未动** | 72h 报告未入库、`.service` 仍为 0、tag 仍只有 v0.1.0 |

**额外**：迭代 2 的 D1 第三方扩展验收已闭环——19.5min/8 摩擦点 → 修复后 8m48s/2 摩擦一次全过（`03f57af`）；"可扩展"从唯一无法自证的词变成有数据的主张。

### 8.2 更新评分：6.5 → 7.5 / 10

| 维度 | 08-25 | 08-28 | 变化原因 |
|---|:---:|:---:|---|
| 测试 | 6.5 | **8** | e2e 落地 + 度量链可复算可信 |
| 安全 | 3 | **7** | P0 收官 + 防再犯在位；DDS security 仍未启用、OTA 签名仍恒真桩 |
| 文档诚实度 | 7 | 8 | 徽章实测化、整改复盘入库 |
| 架构 / 代码质量 | 8.5 / 7.5 | 不变 | 27 处裸 `QoS(10)` 仍在（AmrNode 迁移了节点但 QoS 未清） |
| 可靠性 / 发布工程 | 7.5 / 4 | 不变 | 72h soak、systemd、版本对齐、告警规则均未动 |

### 8.3 剩余缺口（距"单机商用试点"，按优先序）

1. DDS security 真启用——launch 语法修了，但生产 launch/compose 仍无安全变量；
2. OTA 签名真实现（`ota_agent_node.cpp` 恒真桩 → 真实验证）；
3. 72h soak 正式跑出报告入库（A2 尾款）；
4. 告警规则（Prometheus rules / Alertmanager——观测仍只看不叫）；
5. 真机验证（全部验证仍在仿真，A1/Orin 依赖）；
6. 发布工程：systemd unit、版本三方对齐、依赖锁定；
7. 治理尾款：27 处裸 `QoS(10)` 清零、README Status 表手写 84.5% 删除（徽章已 89.3%，**这是第四次数字漂移的苗头**）。

### 8.4 复审结论

- **开源工程规范演示定位（README 第一定位）：已基本达成**——可信度三要素（度量、安全底线、e2e）全部补齐。
- **单机商用试点**：约 2-3 个月全职当量判断不变，缺口全部是已排期或依赖硬件的**已知项**，无未知风险。
- L7 点评："审计→全量处置→防再犯→处置记录入库"的闭环速度与完整度是本仓库新的最强工程证据——它把首次审计诊断的根因（文档先行于实现）反转为"声明先于达成、达成后有验证"的健康模式。注意唯一回潮信号：度量自动化了，人手写的 Status 表格又漂了——**删手写数字，只留徽章**。
