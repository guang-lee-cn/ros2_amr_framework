...
- **踩坑（本地实跑的盲区）**: 首推即红——`KeyError: 'GITHUB_TOKEN'`。
  `GITHUB_TOKEN` **不是**默认环境变量，必须在 `env:` 显式映射（本仓库
  secrets-scan 早就这么写了）；本地实跑时我在命令行手工传了它，恰好把这个
  接缝从被测面里抹掉。教训：本地复现时**只该喂 CI 真会给的输入**，否则验的是
  另一套环境。修完首推 CI 自己就把它证了——这正是本 job 存在的意义。
...
## 2026-09-07 — 第四审与 W1/W2 整改（断更一周补记）

> 断更说明：09-01 三审收官至本条之间，仓库发生了最大规模的一周变更
> （NAV2 收敛 v2.4.0、A/B、L3 安全闸、定位形态、72h soak、v2.5.0 移除）
> 而本 journal 零记录——四审 N/G 项点名。本条为补记，此后恢复逐批记录。

- 四审（20260907-external-l7-audit-r4.md）：三 P1 + 治理四项全部核验成立
